...
static int rcc_set_flash_latency(uint32_t latency) {
  ...
  if ((FLASH->ACR.reg & FLASH_ACR_LATENCY_Msk) != latency) {
    return -7;
  }
  return 0;
}
...
/**
 * @brief  Internal helper: switch system clock source and wait for
 * confirmation.
...
static int rcc_switch_sysclk(uint32_t sw_value) {
  ...
  while ((RCC->CFGR.reg & RCC_CFGR_SWS_Msk) != expected_sws) {
    if (--timeout == 0U) {
      return -8;
    }
  }
...
int rcc_sys_clk_config(const RCC_ClkInit_t *cfg) {
  ...
  if (cfg->sysclk_src == RCC_SYSCLK_PLL) {
     ...
     */
    if ((RCC->CFGR.reg & RCC_CFGR_SWS_Msk) == RCC_CFGR_SWS_PLL) {
      /* Make sure HSI is on */
      RCC->CR.reg |= RCC_CR_HSION;
      timeout = RCC_TIMEOUT;
      while (!(RCC->CR.reg & RCC_CR_HSIRDY)) {
        if (--timeout == 0U)
          return -1;
      }
      /* Switch to HSI */
     ...
     */
    RCC->APB1ENR.reg |= RCC_APB1ENR_PWREN;
    (void)RCC->APB1ENR; /* dummy read — bus sync delay */
    PWR->CR.reg = (PWR->CR.reg & ~PWR_CR_VOS_Msk) | PWR_CR_VOS_SCALE1;

    /* Step 5: Configure PLL (write the entire PLLCFGR register) */
    RCC->PLLCFGR.reg =
        (cfg->pll.PLLM << RCC_PLLCFGR_PLLM_Pos) |
...
uint32_t rcc_get_sysclk_freq(void) {
  uint32_t sws = RCC->CFGR.reg & RCC_CFGR_SWS_Msk;
  ...
  case RCC_CFGR_SWS_PLL: {
    uint32_t pllcfgr = RCC->PLLCFGR.reg;
    ...
    uint32_t pllp =
        (((pllcfgr & RCC_PLLCFGR_PLLP_Msk) >> RCC_PLLCFGR_PLLP_Pos) + 1U) * 2U;
...
void rcc_ahb1_clk_enable(uint32_t periph) {
  RCC->AHB1ENR.reg |= periph;
  (void)RCC->AHB1ENR; /* dummy read — ensures bus write completes (ST errata) */
}

void rcc_ahb1_clk_disable(uint32_t periph) { RCC->AHB1ENR.reg &= ~periph; }

void rcc_apb1_clk_enable(uint32_t periph) {
  RCC->APB1ENR.reg |= periph;
  (void)RCC->APB1ENR;
}

void rcc_apb1_clk_disable(uint32_t periph) { RCC->APB1ENR.reg &= ~periph; }

void rcc_apb2_clk_enable(uint32_t periph) {
  RCC->APB2ENR.reg |= periph;
  (void)RCC->APB2ENR;
}

void rcc_apb2_clk_disable(uint32_t periph) { RCC->APB2ENR.reg &= ~periph; }
...
void rcc_ahb1_periph_reset(uint32_t periph) {
  RCC->AHB1RSTR.reg |= periph; /* assert reset  */
  __asm volatile("dsb" ::: "memory");
  RCC->AHB1RSTR.reg &= ~periph; /* release reset */
}

void rcc_apb1_periph_reset(uint32_t periph) {
  RCC->APB1RSTR.reg |= periph;
  __asm volatile("dsb" ::: "memory");
  RCC->APB1RSTR.reg &= ~periph;
}

void rcc_apb2_periph_reset(uint32_t periph) {
  RCC->APB2RSTR.reg |= periph;
  __asm volatile("dsb" ::: "memory");
  RCC->APB2RSTR.reg &= ~periph;
}
